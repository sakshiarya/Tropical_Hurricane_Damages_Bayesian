...
filep<-'/Volumes/GoogleDrive-116944872060578867313/My\ Drive/Research/2022/Ansu/BayesStorm/Tropical_Hurricane_Damages_Bayesian/2024\ Redo\ Seasonal/Posterior\ Prediction/2020'
#source(paste(filep,'0_Data_Processing.r',sep=''))
source(file.path(filep,'1a_Hyperpar_Beta19.R'))
source(file.path(filep,'1b_Hyperpar_Theta19.R'))
source(file.path(filep,'1c_Hyperpar_Mu_Sigma19.r'))
...
filemcmc<-'/Volumes/GoogleDrive-116944872060578867313/My\ Drive/Research/2022/Ansu/BayesStorm/Tropical_Hurricane_Damages_Bayesian/2024\ Redo\ Seasonal/MCMC'


#Storm-wise data
Storm_Data <-read.csv(file.path(filedata,'Categorized_Storm_1960_2022.csv'))
Storm_Data <- Storm_Data[Storm_Data$Year < 2020,]

#Annualized data
Annual_Data<- read.csv(file.path(filedata,'Categorized_Annual_1960_2022.csv'))
Annual_Data <- Annual_Data[Annual_Data$Year < 2020,]
...
de1 <- data.frame(Year = 1962, Cat_HURDAT = "3-5", Landfall = 0, Freq = 0, damage = NA)
de2 <- data.frame(Year = 2013, Cat_HURDAT = "3-5", Landfall = 0, Freq = 0, damage = NA)
Annual_Data2 <- rbind(Annual_Data[Annual_Data$Cat_HURDAT=='3-5',], de1, de2)
...
data.nbpoi.selected <-list(
  ...
  model.mat.x1= cbind(Annual_Cov[,c(5:8)]), 
  model.mat.x2= cbind(Annual_Cov[,c(6:7)]),
  ...
  hyper_Beta1 = matrix(c(rep(0,ncol(model.mat.x1)),rep(1e-2,ncol(model.mat.x1))),nrow=2,byrow=T), 
  hyper_Beta2 = matrix(c(rep(0,ncol(model.mat.x2)),rep(1e-4,ncol(model.mat.x2))),nrow=2,byrow=T), 
...
set.seed(22415)
jags.MCMC.final.FB.select.2019_withSST <-jags(data= data.nbpoi.selected, parameters.to.save= params.nb, model.file= model.bugs.onecov.nb, n.chains= n.chains, n.thin= n.thin, n.iter= n.iter, n.burnin= n.burnin)
save(jags.MCMC.final.FB.select.2019_withSST ,file=file.path(filemcmc,'jags.MCMC.final.FB.select.2019_withSST.rda'))
