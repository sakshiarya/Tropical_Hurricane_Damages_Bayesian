...
Poi1_select <- glm(Freq ~ NAO + SOI + AMO + ANOM.3.4 + Atl_SST, data = GLM_Data,subset= Cat_HURDAT=='TS-2', family= poisson )
...
## Model 6:
NB1_select <-glm.nb(Freq ~ SOI + AMO + ANOM.3.4 + Atl_SST, data= GLM_Data,
                subset= Cat_HURDAT=='TS-2')
...
Poi2_select <- glm(Freq ~ AMO + ANOM.3.4, data= GLM_Data,subset=Cat_HURDAT=='3-5', family=poisson)
...
hyper_Beta1_poi_selected <-matrix(c(coef(Poi1_select),rep(1e-4,6)),nrow=2,byrow=T)
...
hyper_Beta2_poi_selected <-matrix(c(coef(Poi2_select),rep(1e-4,3)),nrow=2,byrow=T)
