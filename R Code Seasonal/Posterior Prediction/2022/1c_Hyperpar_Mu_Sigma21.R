...
#Storm Wise Damage >0
Storm_Data1 <- Storm_Data[complete.cases(Storm_Data$CURRENT.DAMAGE.2022),]
Storm_gt0_Damages <- Storm_Data1[Storm_Data1$CURRENT.DAMAGE.2022 > 0,]
...
tapply(Storm_Data1[Storm_Data1$CURRENT.DAMAGE.2022>0,]$CURRENT.DAMAGE.2022, Storm_Data1[Storm_Data1$CURRENT.DAMAGE.2022>0,]$Cat_HURDAT,function(x){mean(log(x),na.rm=T)})
...
ggplot(Storm_gt0_Damages,aes(x=log(CURRENT.DAMAGE.2022),fill= Cat_HURDAT))+ geom_density(alpha=.25)+xlab('Storm-wise log(Damage)')+ylab('Density')+ scale_fill_manual('Max. Category',values=c("grey","darkblue"))+theme_bw()
