...
#library(jags)
#Run all data creation and hyperparameter code
filep<-'/Volumes/GoogleDrive-116944872060578867313/My\ Drive/Research/2022/Ansu/BayesStorm/Tropical_Hurricane_Damages_Bayesian/2024\ Redo\ Seasonal/Posterior\ Prediction/2022'
...
filemcmc<-'/Volumes/GoogleDrive-116944872060578867313/My\ Drive/Research/2022/Ansu/BayesStorm/Tropical_Hurricane_Damages_Bayesian/2024\ Redo\ Seasonal/Posterior\ Prediction/2022/MCMC'

#Storm-wise data
Storm_Data <-read.csv(file.path(filedata,'Categorized_Storm_1960_2022.csv'))
Storm_Data <- Storm_Data[Storm_Data$Year < 2022,]

#Annualized data
Annual_Data<- read.csv(file.path(filedata,'Categorized_Annual_1960_2022.csv'))
Annual_Data <- Annual_Data[Annual_Data$Year < 2022,]

#Annualized Covariates (May/June Averages)
Annual_Cov <- read.csv(file.path(filedata,'Annual_Covariates_1960_2022.csv'))
Annual_Cov <- Annual_Cov[Annual_Cov$Year < 2022,]
...
save(clones.MCMC.final.FB.all.April2 ,file=file.path(filemcmc,'clones.MCMC.final.FB.all.April2_2024.rda'))
...
set.seed(22415)
jags.MCMC.final.FB.all.2021 <-jags(data= data.nbpoi.selected, parameters.to.save= params.nb, model.file= model.bugs.onecov.nb, n.chains= n.chains, n.thin= n.thin, n.iter= n.iter, n.burnin= n.burnin)
save(jags.MCMC.final.FB.all.2021,file=file.path(filemcmc,'jags.MCMC.final.FB.all.2021.rda'))
