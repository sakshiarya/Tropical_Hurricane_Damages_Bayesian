...
load(paste(filep,'MCMC/clones.MCMC.final.FB.select.April5_withSST.rda',sep=''))
load(paste(filep,'MCMC/jags.MCMC.final.FB.select.April5_withSST.rda',sep=''))

#################################################################
#Dclone Model
#################################################################
summary(clones.MCMC.final.FB.select.April5_withSST)
dcdiag(clones.MCMC.final.FB.select.April5_withSST)
confint(clones.MCMC.final.FB.select.April5_withSST)
coef(clones.MCMC.final.FB.select.April5_withSST)
dcsd(clones.MCMC.final.FB.select.April5_withSST)
...
#rm(list = ls())
#Load results of MCMC
load(paste(filep,'MCMC/clones.MCMC.final.FB.all.April2_2024.rda',sep=''))
load(paste(filep,'MCMC/jags.MCMC.final.FB.all.April2_2024.rda',sep=''))
...
par(mfrow = c(1,1))
#pdf(paste0("r","_acf_BayesF_select.pdf"), height = 6, width = 8)
autocorr.plot(x=jags.posterior[[1]][,c(14)], auto.layout = F, main=substitute(paste('Autocorrelation Function of ',sigma[m]), list(m=c(1,2)[i-12])), cex.lab=1.5, cex.main=1.5)
#dev.off()
...
par(mfrow = c(1,2))
traceplot(jags.posterior[[1]][,c(14)],main=expression(paste('Trace of ',r)), xlab='Iteration', ylab='Estimate',cex = 0.5, cex.axis = 1, cex.lab = 1.25, cex.main=1.25)
densplot(jags.posterior[[1]][,c(14)],main=expression(paste('Density of ',r)), xlab='Estimate', ylab='Density', cex = 0.5, cex.axis = 1, cex.lab = 1.25, cex.main=1.25)
#dev.off()
