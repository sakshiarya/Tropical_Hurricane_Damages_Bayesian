...
filefigs <- "/Users/sakshiarya/Library/CloudStorage/GoogleDrive-arya.sakshi44@gmail.com/My\ Drive/Research/2022/Ansu/BayesStorm/Tropical_Hurricane_Damages_Bayesian/2024_retake/R\ Code\ Individual/Predictions/2020Prediction/HalfNormalD_HierEVD"
...
save(results, file = file.path(filefigs,"hierNS_HFNDamages_1e6_allVars_1960to2019.RData"))
...
save(depth_table, file = file.path(filefigs,"DepthTablehierNS_HalfNormalD_allvariables_1e6_1960to2019.RData"))

load(file.path(filefigs,"DepthTablehierNS_HalfNormalD_allvariables_1e6_1960to2019.RData"))
depth_table
...
covs_damages <- colnames(tri_var_sub1[,-which(colnames(tri_var_sub1) %in% c("StartMonth","Year","avgLon","NAO"))])
...
save(results, file = file.path(filefigs,"hierNS_HalfNormalDselecteddat_1e6_1960to2019.RData"))
colnames(results) <- names(pos_init1)
results <- mo1$batch
