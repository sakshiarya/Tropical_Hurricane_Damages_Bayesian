...
save(results, file = file.path(filefigs,"hierNS_HFNDamages_1e6_allVars.RData"))
...
save(depth_table, file = file.path(filefigs,"DepthTablehierNS_HalfNormalD_allvariables_1e6_1960to2022.RData"))
...
covs_damages <- colnames(tri_var_sub1[,-which(colnames(tri_var_sub1) %in% c("StartMonth","avgLon","NAO","AMO"))])
...
save(results, file = file.path(filefigs,"hierNS_HalfNormalDselecteddat_1e6_1960to2022.RData"))
colnames(results) <- names(pos_init1)
results <- mo1$batch
