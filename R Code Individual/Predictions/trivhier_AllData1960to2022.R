...
## Date : 4th August, 2022 edited March 23, 2024
#############################################################################################
rm(list = ls())
library(evd)
library(extRemes)
...
filefigs <- "/Users/sakshiarya/Library/CloudStorage/GoogleDrive-arya.sakshi44@gmail.com/My\ Drive/Research/2022/Ansu/BayesStorm/Tropical_Hurricane_Damages_Bayesian/2024_retake/R\ Code\ Individual/Predictions"
...
data1960to2022 <- data_combined[data_combined$Year < 2023,]
dim(data1960to2022)
tail(data1960to2022)

tri_var<- data1960to2022[,c('minCP','maxWS','CURRENT.DAMAGE.2022')]
...
tri_var$avgLat <- data1960to2022$avgLat
tri_var$avgLon <- data1960to2022$avgLon
tri_var$StartMonth <- data1960to2022$StartMonth
tri_var$Year <- data1960to2022$Year
tri_var$max_Cat <- data1960to2022$max_Cat
dim(tri_var)
tri_var$x1 <- tri_var$logmaxWS
tri_var$x2 <- tri_var$logdamage
tri_var$z1 <- tri_var$logminCP
tri_var[15:21] <- data1960to2022[,17:23]
...
var_gam <- c(rep(1500,3),rep(100,(d1))); alpha_sigx2 <- 1; beta_sigx2 <- 3; a_x2 <- -0.55; b_x2 <- 0.5
a_r <- 0.05; b_r <- 1
...
step_scale <- c(rep(0.005, d1+1), rep(0.006, d1+2),0.007, rep(0.004, d1+2), 0.005,0.006,0.005, 0.006, 0.005, 0.006, 0.005)
length(step_scale)
...
pos_init1 <- pos_init[c(1:(d1+1),(d1+4):((d1+4)+(d1+1)),(2*d1+5 + 3):(2*d1+5 + 3 +(d1+2)),(d1+3):(d1+2),(2*d1+5+2):(2*d1 + 6),(3*d1 + 12):(3*d1 + 11), 3*d1 + 13)]
#pos_init1[c(37,39)] <- -0.4
#pos_init1[c(25:26)] <- c(0,1.5)
names(pos_init1) <- c("Intercept minCP (Z1)", covs1[1:d1], 
                      ...
                      expression(xi[X2]),  expression(sigma[X2]), expression(r))


covs_model <- colnames(tri_var)[c(1:d1,(d1+4),(d1+5))]
length(covs_model)
logpost <- logpost_factory(data = tri_var, priorscale =  priorscale, covs = covs_model)
logpost(pos_init1, model = "Tri_GEV")

set.seed(1706)
mo1 <- metrop(logpost, initial = pos_init1, 1e6, scale = step_scale, model = "Tri_GEV")
mo1$accept
results <- mo1$batch
...
while(t <= reps){
  mo1 <- metrop(logpost, initial = pos_init1, TotalMC/reps, scale = step_scale,  model = "Tri_GEV")
...
save(results, file = file.path(filefigs,"triEVDallvariables_1e6_1960to2022.RData"))

load(paste0(filefigs,"/triEVDallvariables_1e6_1960to2021.RData"))
...
xtable(cbind(post_means, post_sd, names(pos_init1)))
...
save(results, file = file.path(filefigs,"NS_trivar_dependence_1e6_metrop_2024Fulldata.RData"))

colnames(results) <- c(paste0("gamma",10:15), paste0("gamma",20:26),  paste0("gamma",30:37), "xiZ1", "sigmaZ1","xiX1","sigmaX1","xiX2","sigmaX2","r")
#results <- mo1$batch
results_every100 <- results[seq(1, nrow(results), 100),]
head(results_every100)

load(paste0(filefigs,"NS_trivar_log_1e6_metrop_18Jan2023.RData"))
pdf(paste0(filefigs,"TS_trivarNS_25Jan2023_1e6_1to6.pdf"))
# plot(ts(mo1$batch[,1:5]), main = "Time Series plots for MCMC output")
plot(ts(results_every100[,1:6]), main = "Time Series plots for MCMC output")
dev.off()

pdf(paste0(filefigs,"TS_trivarNS_25Jan2023_1e6_7to13.pdf"))
# plot(ts(mo1$batch[,1:5]), main = "Time Series plots for MCMC output")
plot(ts(results_every100[,7:13]), main = "Time Series plots for MCMC output")
dev.off() 

pdf(paste0(filefigs,"TS_trivarNS_25Jan2023_1e6_14to21.pdf"))
plot(ts(results_every100[,14:21]), main = "Time Series plots for MCMC output")
dev.off() 

pdf(paste0(filefigs,"TS_trivarNS_25Jan2023_1e6_22to28.pdf"))
plot(ts(results_every100[,22:28]), main = "Time Series plots for MCMC output")
dev.off() 

pdf(paste0(filefigs,"TS_trivarNS_25Jan2023_1e6_22to28.pdf"))
plot(ts(results_every100[,29:34]), main = "Time Series plots for MCMC output")
dev.off() 


pdf(paste0(filefigs,"TS_trivarNS_25Jan2023_1e6_22to28.pdf"))
plot(ts(results_every100[,35:43]), main = "Time Series plots for MCMC output")
dev.off() 

pdf(paste0(filefigs,"ACF_trivarNS_25Jan2023_1e6_1to6.pdf"))
# plot(ts(mo1$batch[,1:5]), main = "Time Series plots for MCMC output")
acf(results_every100[,1:6], main = "ACF plot")
dev.off()

pdf(paste0(filefigs,"ACF_trivarNS_25Jan2023_1e6_7to10.pdf"))
acf(results_every100[,7:10], main = "ACF plot")
dev.off()

pdf(paste0(filefigs,"ACF_trivarNS_25Jan2023_1e6_11to13.pdf"))
# plot(ts(mo1$batch[,1:5]), main = "Time Series plots for MCMC output")
acf(results_every100[,11:13], main = "ACF plot")
dev.off()

pdf(paste0(filefigs,"ACF_trivarNS_25Jan2023_1e6_14to17.pdf"))
# plot(ts(mo1$batch[,1:5]), main = "Time Series plots for MCMC output")
acf(results_every100[,14:17], main = "ACF plot")
dev.off()

pdf(paste0(filefigs,"ACF_trivarNS_25Jan2023_1e6_18to21.pdf"))
# plot(ts(mo1$batch[,1:5]), main = "Time Series plots for MCMC output")
acf(results_every100[,18:21], main = "ACF plot")
dev.off()

pdf(paste0(filefigs,"ACF_trivarNS_25Jan2023_1e6_22to25.pdf"))
# plot(ts(mo1$batch[,1:5]), main = "Time Series plots for MCMC output")
acf(results_every100[,22:25], main = "ACF plot")
dev.off()

pdf(paste0(filefigs,"ACF_trivarNS_25Jan2023_1e6_26to28.pdf"))
# plot(ts(mo1$batch[,1:5]), main = "Time Series plots for MCMC output")
acf(results_every100[,26:28], main = "ACF plot")
dev.off()

acf(results_every100[,29:33], main = "ACF plot")

acf(results_every100[,34:39], main = "ACF plot")

acf(results_every100[,40:43], main = "ACF plot")
...
names_row <- paste(names(pos_init1), c("", rep("MinCP",10), "", rep("MaxWS",11), "", rep("Damages",12), rep("MinCP",2), rep("MaxWS",2), rep("Damages",2),"r"))
...
save(depth_var, file = file.path(filefigs,"NS_trivar_depth_var_1e6_metrop_1960to2022.RData"))
...
covs_damages <- colnames(tri_var_sub1[,-which(colnames(tri_var_sub1) %in% c("avgLon","AMO", "StartMonth","NAO"))])
...
pos_init1 <- pos_init[c(1:(length(covs_minCP)+1), (length(covs_minCP)+4):(length(covs_minCP)+5), (length(covs_minCP)+8): (length(covs_minCP)+ length(covs_damages)+10),
                        (length(covs_minCP)+3):(length(covs_minCP)+2), (length(covs_minCP)+7):(length(covs_minCP)+6), (length(covs_minCP)+length(covs_damages)+12):(length(covs_minCP)+length(covs_damages)+11), length(pos_init))]
...
names(pos_init1) <- c("Intercept minCP (Z1)", covs_minCP, 
                      ...
                      expression(xi[X2]),  expression(sigma[X2]), expression(r))
length(pos_init1)
pos_init1[length(pos_init1)] <- 0.75
...
priorscale <- c(var_alpha,  
                ...
                alpha_sigx1, beta_sigx1, 
                alpha_sigx2, beta_sigx2,
                a_z1, b_z1,a_x1, b_x1, a_x2, b_x2, a_r, b_r)

length(priorscale)

step_scale <- c(rep(0.01, length(covs_minCP)+1), rep(0.01,2), rep(0.01, length(covs_damages)+3), 0.005,0.006,0.005, 0.006, 0.005, 0.006, 0.005)
length(step_scale)
...
#save(results, file = file.path(filefigs,"hierNSselecteddat_1e6_1960to2021.RData"))
#colnames(results) <- names(pos_init1)
#results <- mo1$batch
...
head(results)
filefigs <- "/Users/sakshiarya/Library/CloudStorage/GoogleDrive-arya.sakshi44@gmail.com/My\ Drive/Research/2022/Ansu/BayesStorm/Tropical_Hurricane_Damages_Bayesian/2024_retake/R\ Code\ Individual/Predictions"
save(results, file = file.path(filefigs,"triHierselecteddat_1e6_1960to2022.RData"))
...
final_posterior_est <- cbind(post_means, post_sd)
rownames(final_posterior_est) <- colnames(results)
xtable(final_posterior_est, digits = 4)
results_every100 <- results[seq(1, nrow(results), 100),]
post_means_every100 <- apply(results_every100,2, mean)
post_sd_every100 <- apply(results_every100,2, sd)
