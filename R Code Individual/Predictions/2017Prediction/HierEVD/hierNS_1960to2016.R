...
logpost(pos_init1, model = "Hier_GEV")

set.seed(2403)
mo1 <- metrop(logpost, initial = pos_init1, 1e6, scale = step_scale, model = "Hier_GEV")
mo1$accept
results <- mo1$batch
...
save(results, file = file.path(filefigs,"hierNSallvariables_1e6_1960to2016.RData"))




load(file.path(filefigs,"hierNSallvariables_1e6_1960to2016.RData"))
...
acf(results_every100[,16:21], cex.main = 0.6)
dev.off()
...
save(depth_table, file = file.path(filefigs,"DepthTablehierNSallvariables_1e6_1960to2016.RData"))

load(file.path(filefigs,"DepthTablehierNSallvariables_1e6_1960to2016.RData"))
depth_table
...
covs_damages <- colnames(tri_var_sub1[,-which(colnames(tri_var_sub1) %in% c("StartMonth","avgLon","SOI", "Year","NAO","AMO","Sunspots"))])
